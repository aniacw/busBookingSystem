...
public class Controller {
    ...
    public void initialize() throws SQLException {
        ...
        for (Data.Row row : data1) {
            int routeId = Integer.parseInt(row.get(1).toString());//ma sens?
    ...
    public void onButtonRemoveBusClicked() {
        ...
        try {
            if (Main.getInstance().getRoutesManager().removeRouteById(id))
                statusBarBusManagement.setText("Route removed successfully");
            else
                statusBarBusManagement.setText("Route with given parameters does not exist");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }

    public void onButtonOrdersClicked() {
        try {
            Data orders = Main.getInstance().getBookingsManager().displayUserOrders(Main.getInstance().getLoginManager().getLoggedUser().getId());
            for (Data.Row row : orders)
    ...
    public void onButtonPrintTicketClicked() {
        ObservableList<Data.Row> selectedOrder = ordersListView.getSelectionModel().getSelectedItems();
        Data.Row line = selectedOrder.get(0);
        int bookingId = (Integer) line.get(1);
        int departureId = (Integer) line.get(3);
        double price = (Double) line.get(4);
        ...
        try {
            Data name = Main.getInstance().getBookingsManager().getName(bookingId);
            for(Data.Row data : name){
                nameString = data.get(1).toString();
            }

            Data d = Main.getInstance().getDeparturesManager().getDate(departureId);
            for( Data.Row data : d) {
                dateFound = data.get(1).toString();
            }

            Data time = Main.getInstance().getDeparturesManager().getTime(departureId);
            for (Data.Row data : time) {
                timeFound = data.get(1).toString();
            }

            Data seats = Main.getInstance().getSeatsManager().getSeatsForBooking(bookingId);
            for(Data.Row data : seats){
                String bookedSeat = data.get(1).toString();
                seatsList.add(bookedSeat);
            }

            for(int i = 0; i < seatsList.size(); i++){
                seatsFound = seatsFound + seatsList.get(i) + ", ";
            }
        ...
        ticketCreator.createTicket(bookingId, nameString, dateFound, timeFound, price, seatsFound);
    ...
    public void onButtonBookBusClicked() {
        ...
        try {
            ...
            bookingsManager.addOrder(name, departureId, fare, Main.getInstance().getLoginManager().getLoggedUser().getId());
